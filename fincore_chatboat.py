...
def clear_user_sensitive_data():
    keys_to_clear = [
        "user_name", "user_mobile", "salary", "messages", "essential_expenses",
        "lifestyle_expenses", "existing_emi", "cibil_score", "deduction_80c",
        "sf_name", "sf_target", "sf_months", "ppf_annual", "dashboard_ready",
        "chat_open", "risk_profile", "expected_return", "home_tab", "step",
        "show_home", "login_mobile_input_page"
    ]
...
if st.session_state.show_home:
    ...
    if st.session_state.home_tab == "ABOUT":
        ...
        <div class='market-momentum-content'>
            <div style='color:#22c55e; font-weight:700; font-size:13px; margin-bottom:6px;'>▲ TOP GAINERS</div>
            ...
            <div style='color:#ef4444; font-weight:700; font-size:13px; margin-bottom:6px;'>▼ TOP LOSERS</div>
        ...
        .monitor-grid {{ display: grid; grid-template-columns: repeat(4, minmax(240px, 1fr)); gap: 18px; width: 92%; margin: 30px auto 0 auto; font-family: Arial, sans-serif; animation: microElasticPop 1.2s cubic-bezier(0.16, 1, 0.3, 1) 0.5s backwards; }}
        .monitor-card {{ background: linear-gradient(180deg, rgba(6, 11, 31, 0.96), rgba(15, 23, 42, 0.92)); border: 1px solid rgba(56, 189, 248, 0.18); box-shadow: 0 18px 40px rgba(1, 12, 36, 0.42), inset 0 1px 0 rgba(255, 255, 255, 0.03); border-radius: 28px; padding: 24px; text-align: left; transition: transform 0.3s ease, box-shadow 0.3s ease, border-color 0.3s ease; }}
        .monitor-card:hover {{ transform: translateY(-4px); box-shadow: 0 26px 55px rgba(0, 245, 212, 0.18), inset 0 1px 0 rgba(255, 255, 255, 0.06); border-color: rgba(56, 189, 248, 0.35); }}
        .monitor-card-title {{ font-size: 18px; color: #7dd3fc; font-weight: 800; margin-bottom: 16px; border-bottom: 1px solid rgba(56, 189, 248, 0.16); padding-bottom: 10px; display: flex; align-items: center; gap: 10px; }}
        .monitor-card-body {{ max-height: 340px; overflow-y: auto; padding-right: 6px; }}
        .monitor-card-body {{ scrollbar-width: none; -ms-overflow-style: none; }}
        .monitor-card-body::-webkit-scrollbar {{ display: none; width: 0; height: 0; }}
        .monitor-card-body ul {{ list-style: none; margin: 0; padding: 0; }}
        .monitor-card-body li {{ margin-bottom: 12px; padding: 10px 12px; border-radius: 16px; background: rgba(15, 23, 42, 0.68); border: 1px solid rgba(255, 255, 255, 0.05); }}
        .monitor-card-body li:last-child {{ margin-bottom: 0; }}
        .monitor-card-body span {{ display: inline-block; vertical-align: middle; }}
        .market-momentum-content {{ display: grid; gap: 10px; }}
        .snapshot-metric {{ display: flex; justify-content: space-between; align-items: center; background: rgba(18, 28, 54, 0.65); padding: 14px; border-radius: 18px; margin-bottom: 10px; border: 1px solid rgba(56, 189, 248, 0.12); }}
        .snapshot-val {{ font-weight: 700; color: #f8fafc; font-size: 14px; }}
        .snapshot-delta {{ color: #22c55e; font-size: 13px; font-weight: 700; }}
        .snapshot-delta.negative {{ color: #ef4444; }}

        /* How FinCore Works Flowchart Pipeline Styling */
        .flowchart-container {{ width: 92%; max-width: 1200px; margin: 60px auto 40px auto; padding: 30px 0 8px 0; font-family: Arial, sans-serif; text-align: center; color: white; background: rgba(8, 14, 35, 0.85); border: 1px solid rgba(56, 189, 248, 0.14); border-radius: 32px; box-shadow: 0 20px 40px rgba(0, 0, 0, 0.18); }}
        ...
        .flowchart-pipeline {{ display: flex; align-items: stretch; justify-content: space-between; flex-wrap: nowrap; gap: 18px; padding: 18px; background: rgba(0, 16, 40, 0.55); border-radius: 22px; }}
        .flowchart-node {{ flex: 1 1 0; max-width: 340px; min-width: 260px; min-height: 240px; display: flex; flex-direction: column; justify-content: center; background: rgba(10, 18, 36, 0.95); border: 1px solid rgba(56,189,248,0.25); box-shadow: 0 0 15px rgba(0, 245, 212, 0.12); border-radius: 18px; padding: 22px; transition: all 0.3s ease; }}
        .flowchart-node:hover {{ transform: translateY(-4px); box-shadow: 0 0 30px rgba(0, 245, 212, 0.24); border-color: rgba(56, 189, 248, 0.48); }}
        .node-icon {{ font-size: 36px; margin-bottom: 14px; }}
        .node-heading {{ font-size: 18px; color: #00f5d4; font-weight: 700; margin-bottom: 8px; }}
        .node-desc {{ font-size: 13px; color: #cbd5e1; line-height: 1.5; margin-top: 6px; }}
        .flowchart-arrow {{ font-size: 32px; color: #38bdf8; text-shadow: 0 0 10px rgba(56,189,248,0.6); animation: pulseArrow 1.5s infinite alternate; align-self: center; margin: 0 4px; flex: 0 0 auto; }}
        /* Legacy scroll wrapper removed; monitor-card-body handles scrolling cleanly. */
        ...
        <div class="monitor-grid">
            ...
            <div class="monitor-card">
                ...
                <div class="monitor-card-body monitor-card-body--short">
                    {fd_html_block}
                </div>
        ...
        st.markdown(
            ...
            html::-webkit-scrollbar, body::-webkit-scrollbar, .block-container::-webkit-scrollbar {
                width: 0px;
                background: transparent;
            }
            html, body, .block-container {
        ...
        components.html(home_html, height=1800, scrolling=True)
...
if st.session_state.step == "DASHBOARD" and st.session_state.dashboard_ready:
    ...
    if total_burn > salary:
        ...
        dash_title_col, pdf_btn_col, logout_btn_col = st.columns([3, 1, 1])
        with dash_title_col:
        ...
        with logout_btn_col:
            if st.button("🔓 Logout", use_container_width=True, key="dashboard_logout_btn"):
                logout_and_reset()
                st.success("Logged out successfully. All session data has been removed.")
